...
func (j *Jenkins) NewCredentialsManager(folders ...string) *CredentialsManager {
	return &CredentialsManager{
		J:      j,
		Folder: strings.Join(folders, "/job/"),
	}
}
...
func (cm CredentialsManager) List(domain string) ([]string, error) {
	...
	ids := make([]string, 0)
	err := cm.handleResponse(cm.J.Requester.Get(cm.fillURL(credentialsListURL, domain), nil, &idsResponse, listQuery))
	if err != nil {
		return ids, err
	}
...
func (cm CredentialsManager) GetSingle(domain string, id string, creds interface{}) error {
	//str := ""
	//err := cm.handleResponse(cm.J.Requester.Get(cm.fillURL(configCredentialURL, domain, id), nil, &str, map[string]string{}))
	//if err != nil {
	//	return err
	//}
	//return xml.Unmarshal([]byte(str), &creds)
	return cm.handleResponse(cm.J.Requester.Get(cm.fillURL(configCredentialURL, domain, id), nil, &creds, map[string]string{}))
}
...
func (cm CredentialsManager) postCredsXML(url string, creds interface{}) error {
	...
	return cm.handleResponse(cm.J.Requester.PostXML(url, nil, string(payload), cm.J.Raw, map[string]string{}))
}

func (cm CredentialsManager) handleResponse(resp *resty.Response, err error) error {
	if err != nil {
		return err
	}
