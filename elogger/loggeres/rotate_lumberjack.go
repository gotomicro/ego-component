...
// io.esWriter, including the standard library's log package.
//
...
var _ io.WriteCloser = (*rLogger)(nil)

// rLogger is an io.WriteCloser that writes to the specified filename.
//
// rLogger opens or creates the logfile on first Write.  If the file exists and
...
// original name. Thus, the filename you give rLogger is always the "current" log
// file.
//
// Backups use the log file name given to rLogger, in the form
// `name-timestamp.ext` where name is the filename without the extension,
// timestamp is the time at which the log was rotated formatted with the
// time.Time format of `2006-01-02T15-04-05.000` and the extension is the
// original extension.  For example, if your rLogger.Filename is
...
// Write implements io.esWriter.  If a write would cause the log file to be larger
// than MaxSize, the file is closed, renamed to include a timestamp of the
// current time, and a new log file is created using the original log file name.
// If the length of the write is greater than MaxSize, an error is returned.
func (l *rLogger) Write(p []byte) (n int, err error) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
// Rotate causes rLogger to close the existing log file and immediately create a
// new one.  This is a helper function for applications that want to initiate
// rotations outside of the normal rotation rules, such as in response to
// SIGHUP.  After rotating, this initiates compression and removal of old log
// files according to the configuration.
func (l *rLogger) Rotate() error {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.rotate()
}
...
func (l *rLogger) openExistingOrNew(writeLen int) error {
	l.mill()
...
func (l *rLogger) filename() string {
	if l.Filename != "" {
		return l.Filename
	}
...
func (l *rLogger) millRunOnce() error {
...
func (l *rLogger) oldLogFiles() ([]logInfo, error) {
...
func (l *rLogger) timeFromName(filename, prefix, ext string) (time.Time, error) {
...
func (l *rLogger) dir() string {
	return filepath.Dir(l.filename())
}

// prefixAndExt returns the filename part and extension part from the rLogger's
// filename.
func (l *rLogger) prefixAndExt() (prefix, ext string) {
